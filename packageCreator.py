...
"""
import os, errno, datetime, fileinput, subprocess
...
def setVersionDate():
    ...
    file_path = os.path.join(curFolder, "curveUI.py")
...
for f in toMove:
    dst = os.path.join(baseFolder, "curveCreator", f.replace("\\", "/").split("%s/" % curFolder.replace("\\", "/"))[-1])
    try:
...
def _turnOffDebug():
    file_path = os.path.join(baseFolder, "curveCreator/mayaUtils.py")
    with fileinput.input(file_path, inplace=True) as f:
        for line in f:
            if "_DEBUG = True" in line:
...
subprocess.call(['7z', 'a', os.path.join(baseFolder, "CurveTools_%s.7z" % _vers), os.path.join(baseFolder, "curveCreator")])
subprocess.call(['7z', 'a', os.path.join(baseFolder, "CurveTools_%s.7z" % _vers), _baseINI])
subprocess.call(['7z', 'a', os.path.join(baseFolder, "CurveTools_%s.7z" % _vers), os.path.join(baseFolder, "dragDropInstall.mel")])
subprocess.call(['7z', 'a', os.path.join(baseFolder, "CurveTools_%s.7z" % _vers), os.path.join(baseFolder, "packageInstaller.py")])
